...
function PurchaseDetailsModal({ isOpen, onClose, revenueRecord }: PurchaseDetailsModalProps) {
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-blue-50 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-gray-900 font-mono text-sm">{revenueRecord.userId}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-green-50 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-sm font-medium text-gray-500">Tutor Name</label>
                <p className="text-gray-900">{revenueRecord.tutorName}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-500">Course ID</label>
                <p className="text-gray-900 font-mono text-sm">{revenueRecord.courseId}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-purple-50 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-gray-900 font-mono text-sm">{revenueRecord.paymentId}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-gray-900">
                  {new Date(revenueRecord.paymentDate).toLocaleDateString('en-US', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
                    hour: '2-digit',
              ...
              <div>
                ...
                >
                  {revenueRecord.status.charAt(0).toUpperCase() + revenueRecord.status.slice(1)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-yellow-50 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="text-center">
                ...
                <p className="text-2xl font-bold text-gray-900">₹{revenueRecord.price.toFixed(2)}</p>
              </div>
              <div className="text-center">
                <label className="text-sm font-medium text-gray-500">Admin Commission</label>
                <p className="text-2xl font-bold text-green-600">₹{revenueRecord.adminCommission.toFixed(2)}</p>
              </div>
              <div className="text-center">
                <label className="text-sm font-medium text-gray-500">Tutor Earnings</label>
                <p className="text-2xl font-bold text-blue-600">₹{revenueRecord.tutorEarnings.toFixed(2)}</p>
              </div>
            </div>
            <div className="mt-4 p-3 bg-white rounded border text-center">
              <p className="text-sm text-gray-600">
                Commission Rate: {((revenueRecord.adminCommission / revenueRecord.price) * 100).toFixed(1)}%
              </p>
            </div>
          </div>
        </div>
...
function RevenueManagement() {
  ...
  const transformRevenueData = (revenueData: AdminRevenueData): RevenueRecord => {
    return {
      id: revenueData._id,
      ...
      status: revenueData.status as "completed" | "pending" | "failed" | "refunded",
    };
  };
  ...
  const fetchRevenues = useCallback(async () => {
    try {
      ...
      const response = await getAdminRevenue(currentPage, debouncedSearch, statusFilter, dateFilter);
  ...
  if (error) {
    return (
      <div className="p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Revenue Management</h1>
        </div>
        <div className="bg-red-50 border border-red-200 rounded-md p-4">
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <button onClick={fetchRevenues} className="text-sm text-red-800 underline mt-2">
                Try again
              </button>
            </div>
          </div>
  ...
  return (
    <div className="p-6">
      {/* Header with Stats */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-4">Revenue Management</h1>
          ...
          <div className="bg-white p-4 rounded-lg shadow border">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-green-600">₹{totalCommission.toFixed(2)}</p>
              </div>
            </div>
          </div>
