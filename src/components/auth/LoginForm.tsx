...
import { tutorLogin, googleTutorLogin } from "../../redux/actions/tutorActions";
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    try {
      if (role === "user") {
        ...
        const result = await dispatch(
          googleTutorLogin(credentialResponse.credential)
        ).unwrap();
        toast.success(`Welcome ${result.tutor?.name}!`);
      }
      onLoginSuccess();
    } catch (error: any) {
