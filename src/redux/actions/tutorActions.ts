...
import { TutorSignup,VerifyOtpArgs ,TutorLogin } from "../../interfaces/tutorInterface";

export const tutorSignUp = createAsyncThunk(
    "tutor/signUp",
    async ({ name, email, phone, password }: TutorSignup, { rejectWithValue }) => {
      try {
  ...
  export const tutorVerifyOtp = createAsyncThunk(
    "tutor/verifyOtp",
    async ({ name, email, phone, password, otp }: VerifyOtpArgs, { rejectWithValue }) => {
      try {
        const result = await tutorVerifyOtpService(name, email, phone, password, otp);
        if (result.success) {
          return result.data;  // Return the data if success is true
        } else {
