...
class TokenManager {
  ...
  static setTokens(role: 'admin' | 'tutor' | 'user', accessToken: string, userData?: any) {
    localStorage.setItem(this.TOKEN_KEYS[role], accessToken);
    
    if (userData && role !== 'admin') {
      localStorage.setItem(this.USER_DATA_KEYS[role as 'tutor' | 'user'], JSON.stringify(userData));
    }
  }
...
const createLoginService = (
  instance: typeof adminAxiosInstance, 
  ...
  try {
    ...
    if (response.data.success && response.data.data) {
      ...
      if (role === 'tutor' && userData) {
        console.log("Verification status:", userData, userData.isVerified, userData.verificationStatus);
      }
    }
...
export const adminLoginService = createLoginService(adminAxiosInstance, 'admin');
export const tutorLoginService = createLoginService(tutorAxiosInstance, 'tutor');
...
const createSignUpService = (instance: typeof userAxiosInstance) => 
  async (name: string, email: string, phone: string, password: string): Promise<ApiResponse> => {
    try {
...
const createVerifyOtpService = (instance: typeof userAxiosInstance) =>
  async (name: string, email: string, phone: string, password: string, otp: string): Promise<ApiResponse> => {
    try {
...
const createForgotPasswordService = (instance: typeof userAxiosInstance) =>
...
export const forgotPasswordService = createForgotPasswordService(userAxiosInstance);
export const tutorForgotPasswordService = createForgotPasswordService(tutorAxiosInstance);


const createVerifyForgotOtpService = (instance: typeof userAxiosInstance) =>
  async (email: string, otp: string, isForgot: boolean = true): Promise<ApiResponse> => {
    try {
      const response = await instance.post("/verify-otp", {
        email,
        otp,
...
export const verifyForgotOtpService = createVerifyForgotOtpService(userAxiosInstance);
export const tutorVerifyForgotOtpService = createVerifyForgotOtpService(tutorAxiosInstance);


const createPasswordChangeService = (instance: typeof userAxiosInstance) =>
  async (email: string, password: string): Promise<ApiResponse> => {
    try {
...
export const passwordChangeService = createPasswordChangeService(userAxiosInstance);
export const tutorPasswordChangeService = createPasswordChangeService(tutorAxiosInstance);
